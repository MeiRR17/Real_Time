...
class MockGeneratorFactory:
    ...
    _generators = {
        ...
        "tgw": TGWMockGenerator,
        "sbc": SBCMockGenerator,
        "expressway": ExpresswayMockGenerator
    }
...
class UnifiedGeneratorFactory:
    ...
    @classmethod
    def create_all_generators(cls, server_configs: List[ServerConfig]) -> List[MockDataGenerator]:
        """
        Create generators for all server configurations based on feature toggles.
        
        Args:
            server_configs: List of server configurations
        
        Returns:
            List[MockDataGenerator]: List of generators (mock or real)
        """
        ...
        for config in server_configs:
            ...
            # Determine if we should use real API for this server type
            use_real = False
            if server_type == "cucm":
                use_real = settings.cisco.use_real_cucm
            elif server_type == "uccx":
                use_real = settings.cisco.use_real_uccx
            elif server_type == "cms":
                use_real = settings.cisco.use_real_cms
            elif server_type == "imp":
                use_real = settings.cisco.use_real_imp
            elif server_type == "meeting_place":
                use_real = settings.cisco.use_real_meeting_place
            elif server_type == "tgw":
                use_real = settings.cisco.use_real_tgw
            elif server_type == "sbc":
                use_real = settings.cisco.use_real_sbc
            elif server_type == "expressway":
                use_real = settings.cisco.use_real_expressway
            
            # Create generator for this specific node
            generator = cls.create_generator(server_type, config, use_real)
...
def get_default_server_configs() -> List[ServerConfig]:
    """
    Get default server configurations for all supported types with multi-node support.
    
    Returns:
        List[ServerConfig]: Flattened list of server configurations for all nodes
    """
    settings = get_settings()
    configs = []
    
    # CUCM nodes
    for i, node in enumerate(settings.cisco.cucm_nodes, 1):
    ...
    # UCCX nodes
    for i, node in enumerate(settings.cisco.uccx_nodes, 1):
        configs.append(ServerConfig(
            name=f"UCCX-Node-{i}",
            ip_address=node,
            server_type="uccx",
    ...
    # CMS nodes
    for i, node in enumerate(settings.cisco.cms_nodes, 1):
        configs.append(ServerConfig(
            name=f"CMS-Node-{i}",
            ip_address=node,
            server_type="cms",
    ...
    # IMP nodes
    for i, node in enumerate(settings.cisco.imp_nodes, 1):
        configs.append(ServerConfig(
            name=f"IMP-Node-{i}",
            ip_address=node,
            server_type="imp",
    ...
    # Meeting Place nodes
    for i, node in enumerate(settings.cisco.meeting_place_nodes, 1):
        configs.append(ServerConfig(
            name=f"MeetingPlace-Node-{i}",
            ip_address=node,
    ...
    # TGW nodes
    for i, node in enumerate(settings.cisco.tgw_nodes, 1):
        configs.append(ServerConfig(
            name=f"TGW-Node-{i}",
            ip_address=node,
            server_type="tgw",
    ...
    # SBC nodes
    for i, node in enumerate(settings.cisco.sbc_nodes, 1):
        configs.append(ServerConfig(
            name=f"SBC-Node-{i}",
            ip_address=node,
            server_type="sbc",
    ...
    # Expressway nodes
    for i, node in enumerate(settings.cisco.expressway_nodes, 1):
        configs.append(ServerConfig(
            name=f"Expressway-Node-{i}",
            ip_address=node,
...
# =============================================================================
def create_mock_generators() -> List[MockDataGenerator]:
    """
    Create mock generators for all default server types.
    
    Returns:
        List[MockDataGenerator]: List of mock generators
    """
    server_configs = get_default_server_configs()
    return UnifiedGeneratorFactory.create_all_generators(server_configs)


def create_generators() -> List[MockDataGenerator]:
    """
    ...
    Returns:
        List[MockDataGenerator]: List of generators (mock or real based on toggles)
    """
    server_configs = get_default_server_configs()
    return UnifiedGeneratorFactory.create_all_generators(server_configs)
